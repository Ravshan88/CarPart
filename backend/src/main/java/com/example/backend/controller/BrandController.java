...
public class BrandController {
    ...
    public HttpEntity<?> getBrands(@RequestParam(defaultValue = "") String name) {
        return brandService.getBrands(name);
    }

    @DeleteMapping("/delete/{id}")
    public HttpEntity<?> deleteCarBrand(@PathVariable UUID id, @RequestParam UUID attachmentId, @RequestParam String attachmentName) {
        return brandService.deleteCarBrand(id, attachmentId, attachmentName);
    }
