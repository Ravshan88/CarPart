...
public class CarServiceImpl implements CarService {
    ...
    @Override
    public HttpEntity<?> deleteCar(UUID id, UUID attachmentId, String attachmentName) {
        String folderPath = "files/carPhotos/" + attachmentName;
    ...
    private void createFile(MultipartFile photo, Car existingCar) throws IOException {
        ...
        String filePath = "files/carPhotos/" + fileName;
        File file = new File(filePath);
        file.getParentFile().mkdirs();
        try (OutputStream outputStream = new FileOutputStream(file)) {
            FileCopyUtils.copy(photo.getInputStream(), outputStream);
