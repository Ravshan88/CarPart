...
public class BrandServiceImpl implements BrandService {
    ...
    @Override
    public HttpEntity<?> getBrands(String name) {
        return ResponseEntity.ok(brandRepository.findByNameContainingIgnoreCase(name));

    }

    @Override
    public HttpEntity<?> deleteCarBrand(UUID id, UUID attachmentId, String attachmentName) {
        String folderPath = "files/brandPhotos/" + attachmentName;
    ...
    private void createFile(MultipartFile photo, Brand existingBrand) throws IOException {
        ...
        String filePath = "files/brandPhotos/" + fileName;
        File file = new File(filePath);
        file.getParentFile().mkdirs();
        try (OutputStream outputStream = new FileOutputStream(file)) {
            FileCopyUtils.copy(photo.getInputStream(), outputStream);
