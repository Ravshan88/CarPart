...
import {
    editOperator,
    addOperator, getOperatorData, setIsEditing
...
import {addAdmin} from "../../../redux/reducers/AdminAdminSlice";

function AdminOperators() {
    const dispatch = useDispatch();
    const {operators, isLoading, isEditing} = useSelector((state) => state.adminOperators);
            ...
            if (formData.phone.length === 13) {
                if (isEditing) {
                    if (formData.password.length >= 8) {
                        dispatch(addOperator({formData, isEditing}));
                        closeModal()
                    } else {
                        toast.error(
            ...
            <div className={'umodal'}>
                <Modal show={isModalOpen} onHide={closeModal}>
                    <Modal.Header closeButton>
                        <Modal.Title> Operator qo`shish </Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(handleAddOperator)}>
                            <div>
                                <label htmlFor=""
                                       className="block mb-1 text-sm font-medium text-gray-900 dark:text-white">
                                    Operator Ism Familiyasi</label>
                                <Controller
                                    name='name'
                                    control={control}
                                    defaultValue=""
